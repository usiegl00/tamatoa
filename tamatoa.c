...
typedef bool (*NSUnLinkModule_ptr)(NSModule module, uint32_t options);
typedef bool (*NSDestroyObjectFileImage_ptr)(NSObjectFileImage objectFileImage);
...
int main()
{
  ...
  NSUnLinkModule_ptr NSUnLinkModule_func = (void*)find_symbol(dyld, "_NSUnLinkModule");
  if (!NSUnLinkModule_func) {
    return 1;
  }

  NSDestroyObjectFileImage_ptr NSDestroyObjectFileImage_func = (void*)find_symbol(dyld, "_NSDestroyObjectFileImage");
  if (!NSDestroyObjectFileImage_func) {
    return 1;
  }
  ...
  if (!sierra) {
    ...
    NSUnLinkModule_func -= DYLD_BASE_ADDR;
    NSDestroyObjectFileImage_func -= DYLD_BASE_ADDR;
  }
  ...
  *(char*)buffer = '\xcf';
  ...
  f = NSAddressOfSymbol_func(sym_main);
  ...
  NSUnLinkModule_func(nm, NSUNLINKMODULE_OPTION_NONE);

  NSDestroyObjectFileImage_func(fi);
